...
const modelClassConfig: Record<string, { label: string; color: string; icon: string }> = {
  frontier: { label: "Frontier", color: "bg-purple-500/20 text-purple-400 border-purple-500/30", icon: "🚀" },
  mid: { label: "Mid-tier", color: "bg-blue-500/20 text-blue-400 border-blue-500/30", icon: "⚡" },
  small: { label: "Small", color: "bg-green-500/20 text-green-400 border-green-500/30", icon: "💨" },
  embedding: { label: "Embedding", color: "bg-amber-500/20 text-amber-400 border-amber-500/30", icon: "📊" },
  fine_tuned: { label: "Fine-tuned", color: "bg-pink-500/20 text-pink-400 border-pink-500/30", icon: "🎯" },
};

function AgentCard({ agent, allAgents }: { agent: AgentSpec; allAgents: AgentSpec[] }) {
    ...
    <div className="rounded-lg border border-[var(--border)] bg-[var(--background)] overflow-hidden">
      ...
      >
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[var(--accent)]/10 text-lg">
            {modelConfig.icon}
          </div>
          <div>
