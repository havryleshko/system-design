...
@lru_cache(maxsize=4)
def make_brain(model: str | None=None) -> ChatXAI:
    model_name = model or os.getenv("XAI_MODEL", "grok-4-fast")
...
def call_brain(messages: list[BaseMessage]) -> str:
    r = BRAIN.invoke(messages)
